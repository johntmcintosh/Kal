...
#define KalDataSourceChangedNotification @"KalDataSourceChangedNotification"

@interface JTKalViewController ()
@property (nonatomic, retain, readwrite) NSDate *initialDate;
@property (nonatomic, retain, readwrite) NSDate *selectedDate;
- (KalView*)calendarView;
@end

@implementation JTKalViewController
...
- (void)loadView
{
    ...
    CGSize defaultSize = CGSizeMake(322, 310);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (keyPath==@"frame") {
        CGSize kalSize = self.calendarView.gridView.frame.size;
        self.contentSizeForViewInPopover = CGSizeMake(kalSize.width, kalSize.height + 44.0f);
    }
}
...
- (void)dealloc
{
    ...
    [self.calendarView.gridView removeObserver:self forKeyPath:@"frame" context:nil];
}
